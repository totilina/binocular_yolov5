...
from utils.torch_utils import select_device, smart_inference_mode, model_info, get_num_params, get_flops


from thop import profile  # 添加thop的导入
...
#         # 计算中心加权区域
#         _, _, h, w = x.shape
#         center_mask = torch.zeros((1, 1, h, w), device=x.device)
#         ch, cw = h//2, w//2
#         rh, rw = int(h*0.4), int(w*0.4)  # 中心区域半径
        
#         center_mask[:, :, ch-rh:ch+rh, cw-rw:cw+rw] = 1
#         weighted_attention = attention * center_mask
        
#         # 应用注意力
#         return x * weighted_attention
...
def run(
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], data[task].replace('vis', 'ir'),
                                       imgsz,
                                       batch_size,
                                       model.stride,
                                       single_cls,
    ...
    for batch_i, (im, im2, targets, paths, paths2, shapes) in enumerate(pbar):
        ...
        # with dt[1]:
        #     # 添加ROI注意力
        #     roi_att = ROIAttention(im2.size(1)).to(device)
        #     im2_att = roi_att(im2)
            
        #     # 融合处理
        #     im = torch.cat((im, im2_att), dim=1)
        #     im = im[:, :4, :, :]
        ...
        #     # 确保红外图像是单通道
        #     if im2.shape[1] == 3:
        #         im2 = im2[:, 0:1, :, :]  # 如果是RGB形式，取第一个通道作为红外数据
        #     # 裁剪红外图像中心有效区域
        #     _, _, h, w = im2.shape
        #     crop_size = 0.5  # 裁剪比例
        #     crop_h, crop_w = int(h * crop_size), int(w * crop_size)
        #     start_h, start_w = (h - crop_h) // 2, (w - crop_w) // 2
        #     im2_cropped = im2[:, :, start_h:start_h+crop_h, start_w:start_w+crop_w]
            
        #     # 上采样回原尺寸
        #     im2_cropped = F.interpolate(im2_cropped, size=(h, w), mode='bilinear', align_corners=False)
            
        #     # 融合处理
        #     im = torch.cat((im, im2_cropped), dim=1)
        #     im = im[:, :4, :, :]
        #     preds, train_out = model(im) if compute_loss else (model(im, augment=augment), None)
        ...
        #     # 多尺度融合 - 使用不同大小的特征图
        #     im2_scales = []
        #     for scale in [1.0, 0.75, 0.5]:  # 多尺度处理
        #         scaled_size = (int(h * scale), int(w * scale))
        #         im2_scaled = F.interpolate(im2_cropped, size=scaled_size, mode='bilinear', align_corners=False)
        #         im2_scaled = F.interpolate(im2_scaled, size=(h, w), mode='bilinear', align_corners=False)
        #         im2_scales.append(im2_scaled)
            
        #     # 融合多尺度特征
        #     im2_fused = torch.stack(im2_scales, dim=0).mean(dim=0)
            
        #     # 拼接可见光图像和处理后的红外图像
        #     im = torch.cat((im, im2_fused), dim=1)
        #     im = im[:, :4, :, :]  # 确保只保留4个通道（RGB + IR）
        ...
        # Loss
        if compute_loss:
            loss += compute_loss(train_out, targets)[1]  # box, obj, cls -ori v5
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='73w_6.pt', help='model path(s)')
    ...
    parser.add_argument('--iou-thres', type=float, default=0.36, help='NMS IoU threshold')
