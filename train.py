...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=4, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, imgs2, targets, paths, paths2, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # # 确保红外图像是单通道（如果是3通道则取第一个通道）
            # if imgs2.shape[1] == 3:
            #     imgs2 = imgs2[:, 0:1, :, :]  # 转换为单通道
            ...
            # imgs2_cropped = imgs2[:, :, start_h:start_h+crop_h, start_w:start_w+crop_w]
            
            # # 上采样回原尺寸
            # imgs2_cropped = nn.functional.interpolate(imgs2_cropped, 
            #                                           size=(h, w), 
            #                                           mode='bilinear', 
            #                                           align_corners=False)
            
            # # 拼接可见光图像和处理后的红外图像
            # imgs = torch.cat((imgs, imgs2_cropped), dim=1)
            # imgs = imgs[:, :4, :, :]  # 确保只保留4个通道（RGB + IR）
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/v5s_ai_game.yaml', help='model.yaml path')
    ...
    parser.add_argument('--seed', type=int, default=0, help='Gl512obal training seed')
